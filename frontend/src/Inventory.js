...
//               <tr>
//                 <th className="px-6 py-3 text-left text-xs font-medium text-green-100 uppercase tracking-wider">
//                   Product Info
//                 </th>
//                 <th className="px-6 py-3 text-left text-xs font-medium text-green-100 uppercase tracking-wider">
//                   Category
...
//                             ₦{(parseFloat(product.selling_price) * product.quantity).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
//                           </span>
//                         </div>
...
//                             ₦{(profitPerItem * product.quantity).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
//                           </span>
//                         </div>
...
const Inventory = () => {
  ...
  const [searchTerm, setSearchTerm] = useState('');
  const [currentPage, setCurrentPage] = useState(1);
  const [stockFilter, setStockFilter] = useState('all'); // 'all', 'outOfStock', 'lowStock', 'inStock'
  const itemsPerPage = 10;
  ...
  useEffect(() => {
    ...
    if (stockFilter === 'outOfStock') {
      ...
      filtered = filtered.filter(product => isLowStock(product) && product.quantity > 0);
    } else if (stockFilter === 'inStock') {
      filtered = filtered.filter(product => product.quantity > 0 && !isLowStock(product));
    }
  ...
  const getStockStatus = (product) => {
    if (isOutOfStock(product)) {
      return { status: 'Out of Stock', color: 'red', bgColor: 'red-100', textColor: 'red-800' };
    } else if (isLowStock(product)) {
      return { status: 'Low Stock', color: 'orange', bgColor: 'orange-100', textColor: 'orange-800' };
    } else {
      return { status: 'In Stock', color: 'green', bgColor: 'green-100', textColor: 'green-800' };
    }
  };
  ...
  const getStockCounts = () => {
    const counts = {
      ...
      lowStock: products.filter(product => isLowStock(product) && !isOutOfStock(product)).length,
      inStock: products.filter(product => !isLowStock(product) && !isOutOfStock(product)).length
    };
    return counts;
  };
  ...
  return (
    <div className="p-6 w-full">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-5 gap-6 mb-8">
        ...
        <div className="bg-white rounded-lg shadow-sm p-6 border border-gray-200">
          <div className="flex items-center">
            <div className="text-2xl mr-4 text-orange-500">⚠️</div>
            <div>
              <h3 className="text-sm font-medium text-gray-500">Low Stock</h3>
        ...
        <div className="bg-white rounded-lg shadow-sm p-6 border border-gray-200">
          <div className="flex items-center">
            <div className="text-2xl mr-4 text-yellow-500">⏰</div>
            <div>
              <h3 className="text-sm font-medium text-gray-500">Near Expiry</h3>
              <p className="text-2xl font-bold text-yellow-600">{nearExpiryProductsCount}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm p-6 border border-gray-200">
          <div className="flex items-center">
            ...
            <div>
              <h3 className="text-sm font-medium text-gray-500">Expired</h3>
              <p className="text-2xl font-bold text-red-600">{expiredProductsCount}</p>
            </div>
          </div>
        </div>
      ...
      <div className="bg-white rounded-lg shadow-sm p-6 border border-gray-200 mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-green-50 border border-green-200 rounded-lg p-4">
            <div className="flex items-center justify-between">
              <div>
                <h4 className="text-green-800 font-semibold">In Stock Products</h4>
                <p className="text-green-600 text-sm">Adequate inventory levels</p>
              </div>
          ...
          <div className="bg-orange-50 border border-orange-200 rounded-lg p-4">
            <div className="flex items-center justify-between">
              <div>
                <h4 className="text-orange-800 font-semibold">Low Stock Products</h4>
                <p className="text-orange-600 text-sm">Need restocking soon</p>
              </div>
          ...
          <div className="bg-red-50 border border-red-200 rounded-lg p-4">
            <div className="flex items-center justify-between">
              <div>
                <h4 className="text-red-800 font-semibold">Out of Stock Products</h4>
                <p className="text-red-600 text-sm">Urgent restocking needed</p>
              </div>
        ...
        {stockFilter !== 'all' && (
          <div className="mt-3 p-3 bg-gray-50 rounded-lg border border-gray-200">
            <div className="flex items-center text-sm text-gray-700">
              ...
              Showing {stockFilter === 'inStock' ? 'In Stock' : stockFilter === 'lowStock' ? 'Low Stock' : 'Out of Stock'} products only
              <button
        ...
        <div className="overflow-x-auto">
              ...
              {currentProducts.map((product) => {
                ...
                return (
                  <tr key={product.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    {/* Stock Status */}
                    <td className="px-6 py-4">
                      <span className={`inline-flex px-3 py-1 text-xs font-semibold rounded-full bg-${stockStatus.bgColor} text-${stockStatus.textColor}`}>
                        {stockStatus.status}
                      </span>
                      {outOfStock && (
                    ...
                    <td className="px-6 py-4">
                      <div className="space-y-1">
                        <div className="flex items-center justify-between">
                          <span className="text-sm text-gray-600">Current:</span>
                          <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${outOfStock
                            ? 'bg-red-100 text-red-800'
                            : lowStock
                              ? 'bg-orange-100 text-orange-800'
                              : overstocked
                    ...
                    <td className="px-6 py-4">
                      <div className="flex flex-col space-y-2">
                        ...
                        <button
                          onClick={() => handleStock(product)}
                          className={`${outOfStock ? 'bg-red-500 hover:bg-red-600 text-white' : 'text-green-600 hover:text-green-900 bg-green-50 hover:bg-green-100'} px-3 py-1 rounded transition-colors text-xs`}
                        >
                          {outOfStock ? 'Restock' : 'Stock'}
                        </button>
                        <button
                          onClick={() => handleDelete(product.id)}
                          className="text-red-600 hover:text-red-900 bg-red-50 hover:bg-red-100 px-3 py-1 rounded transition-colors text-xs"
          ...
          {filteredProducts.length === 0 && (
            <div className="text-center py-12">
              <div className="text-4xl mb-4">📦</div>
              <h3 className="text-lg font-semibold text-gray-900 mb-2">
                {searchTerm || stockFilter !== 'all' ? 'No Products Found' : 'No Products Available'}
              </h3>
              <p className="text-gray-500">
                {searchTerm
                  ? `No products found matching "${searchTerm}". Try a different search term.`
                  : stockFilter !== 'all'
                    ? `No ${stockFilter === 'inStock' ? 'in stock' : stockFilter === 'lowStock' ? 'low stock' : 'out of stock'} products found.`
                    : 'Get started by adding your first product to the system.'
                }
              </p>
